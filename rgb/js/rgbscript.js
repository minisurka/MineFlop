...
const presets = [
	...
	['FFA07A', 'FFB347', 'FFD700', '9ACD32', '7FFFD4', '00BFFF', '1E90FF'],
	['00FA9A', '48D1CC', 'C71585'],
	['8B008B', '9370DB', 'FF1493', 'DB7093', 'FF69B4'],
	['228B22', '20B2AA', 'FF8C00', 'FFA500'],
]


// 1. Создаем переменную и задаем формат ПО УМОЛЧАНИЮ (если в URL ничего нет)
// 1. Настройки формата (теперь с нормальными значениями по умолчанию)
let format = {
    template: '&#$1$2$3$4$5$6$f$c', // По умолчанию формат #1
    formatChar: '&',
    maxLength: 256
};

const urlParamesss = new URLSearchParams(window.location.search);
const fParam = urlParamesss.get('format');
...
if (fParam === '1') {
    ...
    format = {
        template: '<#$1$2$3$4$5$6>$f$c',
        formatChar: '&', // Ставим &, чтобы работали bold/italic коды внутри <>
        maxLength: 256
    };
}

// ... (ваши функции getRandomHexColor, Gradient и т.д. остаются без изменений)
...
function updateOutputText(event) {
    ...
    // Заменяем пробелы для расчета градиента
    ...
    for (let i = 0; i < newNick.length; i++) {
        ...
        // ВАЖНО: Берем шаблон из нашего объекта format
        let hexOutput = format.template;
        
        // Заменяем цифры цвета $1, $2... на символы HEX-кода
        for (let n = 1; n <= 6; n++) {
            hexOutput = hexOutput.replace(`$${n}`, hex.charAt(n - 1));
        }

        // Формируем коды стилей (жирный, курсив и т.д.)
        let formatCodes = '';
        ...
        // Заменяем $f на коды стилей и $c на сам символ
    ...
    if (format.maxLength != null) {
        showError(output.length > format.maxLength);
    }
...
function copyTextToClipboard(text) {
	...
	var text = document.getElementById('graylabel2')

	text.textContent = 'Скопировано!'
	setTimeout(() => {
		text.textContent = 'Нажмите на текст, что-бы скопировать'
	}, 1000)
...
class Gradient {
	constructor(colors, numSteps) {
		...
		for (let i = 0; i < colors.length - 1; i++)
			this.gradients.push(
				new TwoStopGradient(colors[i], colors[i + 1], increment * i, increment * (i + 1))
			)
	}
